...
Contribution LTV is the projected lifetime contribution per acquired user before acquisition cost; CAC is the cost of acquiring one user. The MVP must measure engagement and retention alongside coin purchases, content costs, infrastructure, and a small capped paid-acquisition test. The primary business metric is **contribution LTV / CAC**. No universal pass/fail ratio is approved.
...
Each title needs private ownership/component provenance or a complete licensed-rights package plus media acceptance, age/content review and compatible protection. Licensed MVP titles must permit France streaming, Android/Google Play, required English localization, free episodes, coin-based transactional episode access, and paid acquisition using approved clips, trailers, posters, stills and relevant likenesses. Confirm revenue-share reporting, window, takedown, exclusivity restrictions and DRM/protection requirements. See [CONTENT_RIGHTS_CHECKLIST.md](CONTENT_RIGHTS_CHECKLIST.md).
...
- The first-five-episodes free window remains an initial editorial default (D-006), with server/Admin configuration for **free or coin access per episode for MVP**. Preserve dormant ad/both capabilities for later activation, but do not expose ad choices at launch. An operator may choose coin-only cliffhangers; automatic cliffhanger detection is out of scope.
...
- Rewarded ads are post-MVP (D-037). Keep mobile and backend ad modes disabled and show no “watch an ad” option. Future activation requires its own rights, privacy, provider and verified-reward checks; existing ad code need not be removed.
...
coin unlock → continued playback. Reuse the existing app and services. D-037
now defers rewarded ads beyond MVP; required coin economics and release checks
remain. Neither decision authorizes production purchases.
...
2. Locked episode → account creation/login → configured coin cost and current wallet balance → coin unlock or coin-pack selection.
...
Keep Django/DRF modular monolith, Django Admin, React Native/Expo, PostgreSQL, Firebase Auth, Bunny Stream default video delivery, RevenueCat and server-authoritative entitlements. Never serve video bytes through Django.
...
| Advertising | Post-MVP under D-037; in-app ad revenue is not an MVP measurement requirement |
| Commerce | Payer conversion, coin-pack purchase conversion and sales, ARPPU, net IAP revenue/user, refunds/chargebacks |
| Business | Net coin revenue/user, revenue-share and other content cost, contribution LTV, LTV:CAC, economics by series and campaign/creative where supportable |
...
- [x] Founder approves the narrow launch and business experiment, minimum acquisition/economics measurement, and catalog/licensing direction (2026-09-07), narrowed to coin-only monetization on 2026-09-20 (D-037).
...
- [ ] French entity, legal name/form, incorporation, registered address, required registration/organization data, Google Play payments/bank EUR settlement are approved/verified (D-022/D-024/D-025). Apple banking remains an iOS gate.
- [ ] Each title and each acquisition creative passes private rights/provenance, free/coin/promotion, media, age/content and protection review (D-019/D-031).
...
- [ ] Real Android purchase/refund/reconciliation tests pass. Confirm client/server ads are disabled and no ad unlock is offered. AdMob verification (#98) is a future ad-activation gate, not a coin-only MVP launch blocker.
