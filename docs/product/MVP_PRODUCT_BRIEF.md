...
Shortform Streaming is an Android-first platform for a self-owned vertical microdrama series. Viewers discover the series, watch short episodes, and continue through a hardcoded free window plus verified rewarded-ad unlocks.

The MVP is a product-loop validation release. Launch catalog is **one** series. It must determine whether viewers watch through the free window and willingly use verified rewarded ads to continue. Paid-acquisition economics are the next phase, not a reason to add attribution infrastructure now.
...
Founder-approved decisions are marked **Approved**. The release-ready product scope still includes ownership/provenance, GDPR/privacy-by-design, security, quality, age/content controls, consent, account deletion, Google Play policy, server-authoritative ad grants/entitlements, and launch checks. Third-party licensing, contract-driven territorial rights, and multi-country rollout are post-MVP.

Development and validation use local services, emulators/fakes, generated test data, and short self-owned videos. The MVP does not require a licensor, license package, royalty agreement, or contract-driven territory matrix. Company, Google Play organization/account enrollment, AdMob production configuration, payout details, and the France-specific release review remain **Required before public release**, not before architecture or coding.
...
- **Catalog:** exactly **1 self-owned English-language series** at MVP launch — **Founder approved 2026-09-01 (D-004/D-023)**. Ownership and component provenance must be recorded privately. The data model may continue to support N series, but licensed third-party content, licensor contracts, royalties, DRM decisions, and contract-driven territory enforcement are post-MVP.
...
- Record ownership/provenance and delivery metadata for the self-owned series.
- Upload a self-owned master through Django Admin, monitor processing, publish/unpublish, and take down safely.
...
8. Operator → Django Admin upload → provider processing → provenance-checked publication → immediate takedown when required.
...
The authoritative scope is Section 3 of `MICRODRAMA_IMPLEMENTATION_PLAN.md`. In summary, MVP includes one self-owned English series in France on Android, Firebase email/password and Google identity, protected HLS playback, progress, AdMob SSV and server entitlements for ad grants, thin Firebase Analytics events, Django Admin media ingestion, ownership/provenance, staging/prod, CI, privacy, and deletion.
...
It excludes iOS release work, additional countries/languages, licensed third-party content, contract-driven rights/territory workflows, consumer web streaming, user-generated content, offline downloads, live streaming, household profiles, TV apps, custom recommendation ML, and microservices.
...
- Publish only the approved self-owned series, preserve its provenance, and fail closed for unpublished or taken-down media. Legacy generalized tables remain dormant database compatibility only.
...
- Aggregate rewarded-ad yield and contribution for the launch series in France on Android.
...
- [x] Founder approves guest boundary, one self-owned series, hardcoded free window, and rewarded-ad-only monetization defaults (D-004–D-007; catalog scope updated 2026-09-01).
...
- [ ] Founder/content owner records private ownership and component provenance for the exact self-owned launch series; no licensed third-party content is included.
