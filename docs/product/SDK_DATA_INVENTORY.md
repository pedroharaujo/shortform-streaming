...
| D-001 / D-004 / D-023 / D-027 / D-031 | **Founder scope approved 2026-09-02.** MVP is France-only on Android with an English catalog of independently approved self-owned or licensed series. P2-T03-F2 is restoring fixed-market licensed-right enforcement; clients do not select territory/platform/language. Until that task lands, current runtime eligibility remains self-owned-only. |
...
## Current MVP implementation before P2-T03-F2 (2026-09-02)
...
|---|---|---|---|
| Public catalog: opaque ids, direct English title/synopsis, artwork metadata, genre, order, duration and lock state | Currently serves eligible self-owned series in France on Android. Territory, platform, and language are server constants; clients send no market headers. Fixed-market licensed eligibility is approved and in flight under P2-T03-F2. | Django/PostgreSQL | Anonymous catalog readers receive public fields only. Unpublish or takedown hides current titles; P2-T03-F2 must additionally hide licensed rights mismatch/expiry and DRM-required grants before it can merge. |
| `Series.self_owned`, `provenance_reference`, `promotional_use_approved`, `takedown`, `free_episode_count`, `rewarded_ads_enabled`; dormant `ContentRight` rows | Private self-owned publication provenance and the current MVP access policy. P2-T03-F2 will reconnect staff-only licensed grant metadata and opaque private-system references. | Django Admin/PostgreSQL | Staff only except derived eligibility/lock/offer output. Never analytics data. No contracts, rates, provider payloads, personal data, or licensed media enter this repository. |
| `MediaAsset` upload/processing/ready/removed state plus checksum, provider id, captions, thumbnails, duration and renditions | Keep Django Admin ingestion, publication, and playback fail-closed. | Django/PostgreSQL + private upload landing store + Bunny Stream | Staff currently upload approved self-owned masters through Admin. Licensed-master upload remains blocked pending both P2-T03-F2 and the private per-series D-031 clearance. Viewers receive only short-lived authorized playback URLs. |
