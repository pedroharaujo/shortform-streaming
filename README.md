...
Development uses generated/synthetic metadata, self-owned test media and local/emulated/provider-fake integrations. P2-T03-F3 provides generalized server launch configuration and explicit license admission through publication, catalog, playback and ingestion. P3-T01-F1 adds [editorial episode modes, policy versions, and audited Admin changes](docs/runbooks/access-policy.md). P3-T02 supplies the [immutable wallet and atomic unlock](docs/runbooks/coin-wallet.md); the P3-T08-F2 local Android slice adds wallet, configured choices and safe interrupted-request resolution. P3-T04 provides [synthetic verified purchase credits](docs/runbooks/synthetic-purchases.md); the P3-T06 prerequisite adds server-approved synthetic packs and owner-scoped transaction status for future checkout synchronization. Native Google Play checkout, actual provider reconciliation and support/release evidence remain unfinished; production spending and purchases stay disabled. All contracts, suppliers, rates, provider payloads, personal data and licensed media stay outside public Git. Unsupported DRM grants remain ineligible.
